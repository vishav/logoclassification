...
def read_config_values():
    ...
    global batch_size, num_classes, num_epochs, num_gpu, num_workers, image_width, image_height, horizontal_flip
    ...
    num_gpu = int(config['MxnetConfigValues']['num_gpu'])
    ...
    # print("{}:{}:{}:{}:{}:{}:{}".format(batch_size, num_classes, num_epochs, num_gpu, image_width, image_height, horizontal_flip))
...
def show_images(imgs, nrows, ncols, figsize=None):
    ...
    for r in range(nrows):
        for c in range(ncols):
            figs[r][c].imshow(imgs[r*c+1].asnumpy())
...
def _get_batch(batch, ctx):
    ...
    return (split_and_load(data, ctx), split_and_load(label, ctx), data.shape[0] )
