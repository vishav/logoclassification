...
train_rec_file = "./rec-files/train/logoclassification_80_train.rec"
val_rec_file = "./rec-files/train/logoclassification_80_val.rec"
test_rec_file = "./rec-files/test/logoclassification.rec"
...
def evaluate_accuracy(data_iter, net, ctx):
    ...
    for i, batch in enumerate(data_iter):
        ...
        for m in metrics:
            m.update(preds=prediction, labels=label)
...
hybridize=False, learning_rate=0.01, wd=0.002):
    ...
    for epoch in range(num_epochs):
        ...
        for idx, batch in enumerate(train_data):
            ...
            loss.backward()
            print(loss)
            train_loss += sum([loss.sum().asscalar()])
